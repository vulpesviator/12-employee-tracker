...
/* Database connection loads the Start Menu fucnction */
db.connect((err) => {
  if (err) throw err;
  startMenu();
});

/* This function holds the opening inquirer prompts which allow a user to select what actions they would like to take within the database */
startMenu = () => {
  inquirer
    .prompt([
      {
...
/* This query displays a table of all departments */
...
/* This query displays a table of all the roles */
const viewRoles = () => {
...
/* This query displays a table of all employees organized by id, manager, and department based on user input */
const viewEmployees = () => {
  inquirer
    .prompt([
      {
    ...
    .then((answer) => {
      switch (answer.viewOption) {
        ...
        case "By Department":
          db.query(
            `SELECT * FROM employee JOIN role ON employee.role_id = role. id JOIN department ON role.department_id = department.id ORDER BY department.id`,
            (err, res) => {
              if (err) throw err;
              console.table(res);
              startMenu();
...
/* This function allows a user to add a department to the database */
const addDepartment = () => {
  inquirer
    .prompt([
      {
...
/* This function allows a user to add a role/job title to the database */
const addRole = () => {
    ...
    inquirer
      .prompt([
        ...
        },
        {
          type: "input",
          message:
            "Please enter the base salary for this role using only numbers:",
          name: "salary",
          validate: (input) => {
      ...
      .then((answer) => {
        db.query(
          ...
          [answer.roleTitle, answer.salary, answer.departmentId],
          (err, res) => {
            if (err) throw err;
...
/* This function allows a user to add an employee to the database */
const addEmployee = () => {
...
/* This function allows a user to update an existing employee record in the database */
...
/* This function allows a user to remove a department from the database */
    ...
    inquirer
      .prompt([
        ...
        },
        {
          type: "confirm",
          message: chalk.bgRed(
            "Are you sure you want to delete this entire department?"
          ),
          name: "confirmRemove",
          default: false,
        },
...
/* This function allows a user to remove a role/job title from the database */
const removeRole = () => {
  db.query(`SELECT id, title FROM role`, (err, roles) => {
    if (err) throw err;
...
/* This function allows a user to remove an employee from the database */
